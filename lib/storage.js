...
class Storage {
  ...
  async saveData(id, data, options = {}) {
    const { entity, encrypted = false } = options;
    const filePath = path.join(this.#basePath, `${id}.json`);
    const record = encrypted ? encrypt(data, this.#keys.publicKey) : data;
    const raw = JSON.stringify({ entity, data: record, encrypted });
  ...
  async loadData(id) {
    ...
    const { entity, data, encrypted } = JSON.parse(raw);
    const result = encrypted ? decrypt(data, this.#keys.privateKey) : data;
    return { entity, data: result, encrypted };
  }
}
